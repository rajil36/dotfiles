...
function createProgressListeners(client) {
    ...
    const instanceFieldStyle = vscode_1.window.createTextEditorDecorationType({
        color: new vscode_1.ThemeColor('javaFieldColor')
    });
    const staticFieldStyle = vscode_1.window.createTextEditorDecorationType({
        color: new vscode_1.ThemeColor('javaFieldColor'),
        fontStyle: 'italic'
    });
    const mutableVariableStyle = vscode_1.window.createTextEditorDecorationType({
    ...
    vscode_1.window.onDidChangeVisibleTextEditors(updateVisibleDecorations);
    client.onNotification(new vscode_languageclient_1.NotificationType('java/setDecorations'), (event) => {
        fieldDecorations = event;
        updateVisibleDecorations();
    });
}
function asDecoration(r) {
